...
- **Decision:** Rol fijo `SupervisorSoporte` en `Dim_Rol`. `MonitoreoSLAService` resuelve el receptor vía `SupervisorSoporteRepository`: usuarios en `Dim_Usuario_Rol` con ese rol (sin turnos / sin round-robin). Si hay varios, opcionalmente `SOPORTE_SUPERVISOR_USER_ID` elige uno; si no, el menor `idusuario`. El env solo es fallback si aún nadie tiene el rol.
- **Rationale:** Clarificación Session 2026-07-21 + ops 2026-07-29 (no depender de un ID hardcodeado en `.env` para el caso normal).
- **Alternatives considered:**
  - Tabla de turnos nueva (rechazado en clarify — fuera de alcance de este ciclo).
  - Solo `SOPORTE_SUPERVISOR_USER_ID` (rechazado ops: frágil si cambia el usuario supervisor).
