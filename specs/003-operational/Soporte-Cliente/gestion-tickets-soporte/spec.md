...
1. El actor completa el formulario: `idcliente`, `asunto`, `descripcion`, `tipo`, adjuntos (opcional).
...
Job programado que se ejecuta periódicamente:
1. Lee todos los `Fact_Reclamo` con `idestadosoporte` distinto de Cerrado.
2. Compara `fechahora` + `sla_primera_respuesta`/`sla_resolucion` contra la hora actual.
3. **Umbral 80%:** si el tiempo transcurrido supera el 80% del SLA permitido y el ticket no está Resuelto: `Fact_Reclamo` — UPDATE: `sla_status='en riesgo'`. `Fact_Historial_Ticket` — INSERT: `tipo_accion='alerta_sla_riesgo'`.
4. **Límite excedido:** si superó el SLA sin estar Resuelto/Cerrado: `Fact_Reclamo` — UPDATE: `sla_status='incumplido'`, `idestadosoporte=Escalado`, `id_agente_asignado=supervisor/gerente`. `Fact_Historial_Ticket` — INSERT: `tipo_accion='escalado_automatico_sla'`.
5. SLA marcado como `'cumplido'` cuando el ticket se resuelve dentro del plazo (se setea durante `CU-O92`).
...
5. El ticket reabierto vuelve al flujo de `CU-O92` para ser atendido nuevamente.

**Decisión de negocio pendiente:** al reabrir, ¿se asigna un `idslaconfig` nuevo (posiblemente con tiempos más cortos por reincidencia) o se conserva el original?
- **Opción A (conservar):** se mantiene el `idslaconfig` original del ticket.
- **Opción B (renovar):** se busca una nueva configuración vigente en `Dim_SLAConfig` y se actualiza `idslaconfig`/`sla_primera_respuesta`/`sla_resolucion`.
...
El job de `CU-O96` debe ejecutarse con una frecuencia suficiente para detectar el umbral del 80% con margen de reacción útil. **Pendiente de confirmación:** el intervalo exacto (ej. cada 1 o 5 minutos) no está definido en la documentación fuente.
...
### RN-TIC-005
El escalado automático por SLA incumplido asigna al supervisor/gerente de turno como `id_agente_asignado`.
...
`idcliente`, `asunto`, `descripcion`, `tipo`, `adjuntos[]` (opcional).
...
Y debe escalar automáticamente a `idestadosoporte=Escalado` con el supervisor de turno.
...
- **`autenticacion-y-rbac`:** requiere roles Cliente, Soporte al Cliente, Administrador, Desarrollador de APIs, Director Tecnológico.
