...
## 3) Cobertura geográfica de la región (`Dim_RegionOperativaEstadoRegion`)
...
- Es una tabla **puente geográfica** (qué estados/provincias cubre la región), **no** un historial de ciclo de vida.
- **Escritura en CU-O55** al alta (y ensure en reingreso): `idestadoregion` = `Dim_RegionOperativa.idestado`. Consumida por Emergencias para cobertura.
