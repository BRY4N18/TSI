...
**Input**: Design documents from `specs/003-operational/Red-Operativa/incorporacion-regional/backend/`
...
- [X] T003 [Histórico-UI] [P] Confirmar que el módulo Angular `frontend/src/app/modules/red-operativa/` existe y crear la subcarpeta `incorporacion-regional/` con `models/`, `services/`, `guards/`, `pages/`
...
- [X] T019 [Histórico-UI] [P] [US1] Crear modelo TS `RegionOperativa`/`ValidacionRegionRequest`/`Response` en `frontend/src/app/modules/red-operativa/incorporacion-regional/models/region-operativa.contract.ts` (alineado 1:1 al schema OpenAPI)
- [X] T020 [Histórico-UI] [US1] Implementar `RegionOperativaApiService.ejecutarValidacion()` en `frontend/src/app/modules/red-operativa/incorporacion-regional/services/region-operativa-api.service.ts`
- [X] T021 [Histórico-UI] [US1] Implementar `RegionOperativaFacadeService` en `frontend/src/app/modules/red-operativa/incorporacion-regional/services/region-operativa-facade.service.ts` (orquesta llamada + estado de UI, sin lógica de negocio en componentes)
- [X] T022 [Histórico-UI] [US1] Página `frontend/src/app/modules/red-operativa/incorporacion-regional/pages/validacion/` (formulario alta/validación, muestra `estadoregion_actual` tras respuesta)
- [X] T023 [Histórico-UI] [US1] Guard `frontend/src/app/modules/red-operativa/incorporacion-regional/guards/administrador-red-operativa.guard.ts` (reutilizar el de `alta-unidades` si existe, si no crear siguiendo su mismo patrón)
...
- [X] T038 [Histórico-UI] [P] [US3] Guard `frontend/src/app/modules/red-operativa/incorporacion-regional/guards/director-tecnologico.guard.ts`
- [X] T039 [Histórico-UI] [US3] Página `frontend/src/app/modules/red-operativa/incorporacion-regional/pages/reevaluacion/` (degradar/despublicar, protegida por `director-tecnologico.guard.ts`)
...
- [DEFERRED] T047 Ejecutar `quickstart.md` completo (Escenarios A–G) contra docker-compose — checklist ops post-deploy; equivalente cubierto por `pytest -m api` in-memory. Tras seed `scripts/seed_catalogos_red_operativa.py`, validar GET `/regiones`, POST validación, O62 con cobertura=0.
...
- [X] T050 [P] Regla cobertura O62 (`CoberturaRegionReadRepository`) + test p95 RNF-REGON-002 en `tests/performance/test_continuidad_region_p95.py`
- [X] T051 [P] Idempotency-Key cache en escrituras `apps/red_operativa/idempotency.py`
- [X] T052 FE catálogo regiones + nav + links a validación/reevaluación
