...
- Spec y plan en `specs/003-operational/Red-Operativa/incorporacion-regional/backend/`
- Módulo **autenticacion-y-rbac** operativo (login JWT, roles Administrador y Director Tecnológico)
- Un `Dim_EstadoRegion` existente para poblar `idestado` (alta de región nueva) — seed §0
- App `red_operativa` desplegada (compartida con `alta-unidades`, ya implementada)

## 0) Seed ops (Dim_EstadoRegion + rol Unidad)
...
Publica `Dim_EstadoRegion` (CDMX=1, Jalisco=2) y el rol `Unidad` (compartido con alta-unidades O56).
...
### Escenario F — Despublicación automática (O62, CA-REGON-008)

1. Región en `"Producción"`/`"En_Alerta"` **sin** unidades activas en su cobertura (puente → condados) → `POST .../despublicacion-automatica` → `200`, `estadoregion="Despublicada"`.
2. Si aún hay unidades activas en cobertura → `409`.
3. Cron ops: invocar el endpoint por región candidata (no match textual).
