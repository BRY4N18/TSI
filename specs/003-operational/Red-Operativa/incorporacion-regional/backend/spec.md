...
**Fuente de alineación:** `flujoscorreguidos/flujo-red-operativa-canonico.md`.
...
### RN-REGON-005 — Cobertura para CU-O62
La cobertura de una región se agrega así: `Dim_RegionOperativaEstadoRegion.idestadoregion` → `Dim_Condado.idestado` (o `idestadoregion`) → unidades activas con ese `idcondado`. CU-O62 solo despublica si el conteo es 0. No existe FK directa unidad↔región; esta agregación es la regla operativa confirmada.
...
### CA-REGON-008
El sistema expone un servicio de despublicación automática (CU-O62) invocable por Administrador o job/cron **cuando** el conteo de unidades activas en la cobertura de la región es 0. Cobertura = `Dim_RegionOperativaEstadoRegion` → `Dim_Condado` → `Dim_UnidadEmergencia(activo)`. Si aún hay unidades activas → `409`. El auto-trigger del cron es ops (llamar el endpoint por región en `Producción`/`En_Alerta`); no usa match textual frágil.
...
- **Definición de la relación cobertura (`idcondado` / puente geo) ↔ `idregionoperativa`:** **resuelta** en RN-REGON-005 (agregación puente → condado → unidades). El job/cron que invoca el endpoint queda como ops, fuera del runtime de esta app.
