...
- **Rationale**: RN-REGON-005 — no hay FK `Dim_UnidadEmergencia` ↔ `Dim_RegionOperativa`. Tras `alta-unidades`, la cobertura unitaria es `idcondado` (no `zonacobertura`); aún falta la regla de agregación condado/puente geo → región. Priorizar Safety (Principio IX) sobre un disparador heurístico.
- **Alternatives considered**: (a) match frágil texto/`zonacobertura` — obsoleto (campo eliminado); (b) migración de esquema / regla `idcondado`↔región — correcta pero fuera de alcance (Sección 13).
