...
**Input**: Design documents from `specs/003-operational/Red-Operativa/alta-unidades/backend/`
...
- [X] T003 [Bridge-FE] [P] Generar tipos TypeScript del contrato en `frontend/src/app/modules/red-operativa/alta-unidades/models/unidad-emergencia.contract.ts` desde `contracts/alta-unidades.openapi.yaml`
- [X] T004 Crear matriz de trazabilidad CU→RF/RNF/CA→Task IDs en `specs/003-operational/Red-Operativa/alta-unidades/backend/traceability.md`
...
- [X] T005 Validar contrato OpenAPI como gate inicial en `specs/003-operational/Red-Operativa/alta-unidades/backend/contracts/alta-unidades.openapi.yaml`
...
- [X] T023b [Histórico-UI] **[Migración cruzada]** Actualizado `zonacobertura`→`idcondado` en el tipo `DisponibilidadUnidadData` de `frontend/src/app/modules/evidencia-unidad/services/models/evidencia-unidad.types.ts:136`
- [X] T023c [Histórico-UI] **[Migración cruzada]** Actualizado el binding a `idcondado` en `frontend/src/app/modules/evidencia-unidad/pages/panel-disponibilidad/panel-disponibilidad.page.html:62`
- [X] T023d [Histórico-UI] [P] **[Migración cruzada]** Creado test de componente (no existía antes) para el panel de disponibilidad reflejando `idcondado` en `frontend/src/app/modules/evidencia-unidad/pages/panel-disponibilidad/panel-disponibilidad.page.spec.ts`
- [X] T023e [Histórico-UI] **[Migración cruzada]** Backend de `evidencia-unidad` vive en `apps/despacho` (ya validado en T023, 90/90 verde). Frontend: no hay Chrome/Karma en este entorno; validado con `tsc --noEmit` limpio en `tsconfig.app.json` y `tsconfig.spec.json`
...
- [X] T027 [Histórico-UI] [P] [US1] Crear test unitario frontend (marker: unit, AAA) para `UnidadEmergenciaApiService` en `frontend/src/app/modules/red-operativa/alta-unidades/services/unidad-emergencia-api.service.spec.ts`
- [X] T028 [Histórico-UI] [P] [US1] Crear test unitario frontend (marker: unit, AAA) para `AdministradorRedOperativaGuard` en `frontend/src/app/modules/red-operativa/alta-unidades/guards/administrador-red-operativa.guard.spec.ts`
...
- [X] T032 [Histórico-UI] [US1] Implementar `UnidadEmergenciaApiService` en `frontend/src/app/modules/red-operativa/alta-unidades/services/unidad-emergencia-api.service.ts`
- [X] T033 [Histórico-UI] [US1] Implementar `UnidadEmergenciaFacadeService` (orquestación, validaciones de UI) en `frontend/src/app/modules/red-operativa/alta-unidades/services/unidad-emergencia-facade.service.ts`
- [X] T034 [Histórico-UI] [US1] Implementar `AdministradorRedOperativaGuard` en `frontend/src/app/modules/red-operativa/alta-unidades/guards/administrador-red-operativa.guard.ts`
- [X] T035 [Histórico-UI] [US1] Implementar página de catálogo (listado + alta individual) en `frontend/src/app/modules/red-operativa/alta-unidades/pages/catalogo/catalogo.page.ts`
- [X] T036 [Histórico-UI] [US1] Configurar rutas lazy de alta-unidades en `frontend/src/app/modules/red-operativa/alta-unidades/alta-unidades.routes.ts` (registradas en `app.routes.ts`)

**Checkpoint**: US1 operativa como MVP — alta individual con validación de placa.

**US1 Gate (must pass before next story)**:
- [X] T037 [US1] Validar cumplimiento de `CA-CAM-001`, `CA-CAM-002` en `specs/003-operational/Red-Operativa/alta-unidades/backend/traceability.md`
...
- [X] T044 [Histórico-UI] [US2] Extender página de catálogo con importación en lote (input de archivo + reporte de errores) en `frontend/src/app/modules/red-operativa/alta-unidades/pages/catalogo/catalogo.page.ts`

**Checkpoint**: US2 funcional e independiente de US3-US5.

**US2 Gate (must pass before next story)**:
- [X] T045 [US2] Validar cumplimiento de `CA-CAM-003` en `specs/003-operational/Red-Operativa/alta-unidades/backend/traceability.md`
...
- [X] T051 [Histórico-UI] [US3] Implementar página de edición en `frontend/src/app/modules/red-operativa/alta-unidades/pages/edicion/edicion.page.ts`

**Checkpoint**: US3 completa; edición con reglas de bloqueo operativas.

**US3 Gate (must pass before next story)**:
- [X] T052 [US3] Validar cumplimiento de `CA-CAM-004`, `CA-CAM-005` en `specs/003-operational/Red-Operativa/alta-unidades/backend/traceability.md`
...
- [X] T059 [Histórico-UI] [US4] Implementar página de baja en `frontend/src/app/modules/red-operativa/alta-unidades/pages/baja/baja.page.ts` (incluye reactivación)
...
- [X] T060 [US4] Validar cumplimiento de `CA-CAM-006`, `CA-CAM-007` en `specs/003-operational/Red-Operativa/alta-unidades/backend/traceability.md`

---

## Phase 7: User Story 5 - Disponibilidad de unidad externa (Priority: P2) — **SUPERSEDED**

> **Session 2026-07-24 / CA-CAM-009:** CU-O59 eliminado. T061–T070 son históricas;
> el endpoint/FE de disponibilidad en este módulo **no forman parte del producto**. Usar CU-O30.
> No reintroducir sin `/speckit-clarify`.

- [X] T061–T070 ~~[US5]~~ **superseded** — retirados del producto.
...
- [X] T072 [P] Documentar evidencia de performance en `specs/003-operational/Red-Operativa/alta-unidades/backend/quickstart.md`
- [X] T073 [P] Actualizar mapeo RF/RNF/CA→Task IDs en `specs/003-operational/Red-Operativa/alta-unidades/backend/traceability.md`
- [X] T074 [P] Actualizar `module-map.md` marcando `alta-unidades` como "✅ Implementado" en `.specify/docs/architecture/module-map.md`
- [X] T075 Ejecutar validación end-to-end según `specs/003-operational/Red-Operativa/alta-unidades/backend/quickstart.md`: 62/62 tests `red_operativa`, 90/90 `despacho` post-migración, `ng build` de producción exitoso
