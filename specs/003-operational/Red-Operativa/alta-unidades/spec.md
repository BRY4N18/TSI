...
- **CU-O56: Registrar unidades en lote** — Importación CSV/Excel por el Proveedor; valida unidades + credenciales; todo-o-nada; crea login por fila.
...
6. **No** inserta fila en `Fact_HistorialEstadoUnidad` aquí — el estado inicial lo declara la unidad vía **CU-O30** tras recibir credenciales (si O54 también crea login, documentar en plan; el lote O56 sí crea login de forma mandatoria).

### RF-CAM-002: Registro en lote (CU-O56)

El **Proveedor** importa CSV/Excel:
...
### CU-O56
Archivo CSV/Excel: columnas O54 + `gmail` por fila.
