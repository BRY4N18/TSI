...
- `Dim_UnidadEmergencia`: `idunidademergencia` (PK), `idcliente` (FK a `Dim_Cliente`), `tipopropiedad` ('Propia'|'Externa'), `placa`, `capacidad`, `zonacobertura`, `contactoproveedor`, `unidademergencia`, `tipounidademergencia`, `activo`, `latitud`, `longitud`, `fecha_actualizacion`.
...
- `Dim_EstadoUnidadEmergencia`: catálogo de estados (Activa, Ocupada, Fuera de servicio).
...
- `idcliente` (INT, FK a `Dim_Cliente` — la empresa proveedora, si es externa).
- `tipopropiedad` ('Propia' | 'Externa', requerido).
- `placa` (STRING, requerido, único).
- `capacidad`, `zonacobertura` (STRING, opcionales).
...
2. Insertar en `Dim_UnidadEmergencia`.
3. **No** insertar ninguna fila en `Fact_HistorialEstadoUnidad` en este paso — el estado inicial de disponibilidad se establece la primera vez que se declara explícitamente, vía `CU-O30` (módulo Emergencias, si la unidad tiene login propio) o `CU-O59` (este spec, si no lo tiene).
...
El Administrador debe poder editar los campos: `tipopropiedad`, `capacidad`, `zonacobertura`, `contactoproveedor`, `unidademergencia`, `tipounidademergencia`, `latitud`, `longitud`. **No** se puede modificar `idunidademergencia` ni `idcliente`.
...
4. **Reactivación:** si se requiere reactivar una unidad dada de baja, es un `UPDATE` de `Dim_UnidadEmergencia.activo = true`; el registro en `Fact_BajaUnidad` permanece como historial de que hubo una baja previa — no se elimina ni se modifica.
...
1. Insertar en `Fact_HistorialEstadoUnidad`: `idunidademergencia`, `idestadounidademergencia`, `estadoanterior`, `estadonuevo`, `idusuario` = **el operador** que recibió la información y hace la declaración (no el usuario-sistema de la unidad — esta es la diferencia funcional con `CU-O30`), `fechahora`.
...
El procesamiento de un archivo de hasta 500 unidades (CU-O56) debe completarse en menos de 30 segundos, con reporte fila por fila de errores.
...
### RN-CAM-003
`placa` debe ser única entre unidades activas — se valida tanto en alta individual (CU-O54) como en lote (CU-O56).
...
### RN-CAM-005
No existe FK directa entre `Dim_UnidadEmergencia` y `Dim_RegionOperativa` (módulo Red-Operativa) a nivel de estructura de datos — la relación entre `zonacobertura` de la unidad y la región operativa es solo textual, sin validación referencial automática. *(Pendiente de confirmación, documentado igual en `ConfiguracionRedOperativa.md`.)*

## 7. Entradas

### Para registro individual (CU-O54)
`idcliente`, `tipopropiedad`, `placa`, `capacidad`, `zonacobertura`, `contactoproveedor` (si Externa), `unidademergencia`, `tipounidademergencia`, `activo`.
...
- Es requerido por:
  ...
  - **`despacho-inteligente`** (módulo Emergencias): el algoritmo de despacho solo considera unidades con `activo=true`.
