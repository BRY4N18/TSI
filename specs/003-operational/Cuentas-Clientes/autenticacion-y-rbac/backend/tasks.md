...
**Input**: Design documents from `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/`
...
- [X] T001 [Histórico-UI] Crear estructura de carpetas backend/frontend para auth-rbac en `backend/apps/cuentas_clientes/{views,services,tests}` y `backend/core/repositories/cuentas_clientes/` y `frontend/src/app/modules/cuentas-clientes/auth/{pages,services,guards}`
...
- [X] T004 [Bridge-FE] [P] Generar cliente tipado del contrato en `frontend/src/app/modules/cuentas-clientes/auth/services/auth-api.types.ts` desde `contracts/auth-rbac.openapi.yaml`
- [X] T005 Crear matriz de trazabilidad OE/OT/OP/CU->RF/RNF/CA en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
...
- [X] T006 Validar contrato OpenAPI como gate inicial en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/contracts/auth-rbac.openapi.yaml`
...
- [X] T015 [Histórico-UI] Implementar interceptor frontend para Bearer token en `frontend/src/app/core/interceptors/auth.interceptor.ts`
- [X] T016 [Histórico-UI] [P] Crear test unitario (marker: unit, AAA) para `auth.interceptor.ts` en `frontend/src/app/core/interceptors/auth.interceptor.spec.ts`
...
- [X] T018 Registrar decision log para RNF-AUT-005 diferido en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/decision-log.md`
...
- [X] T025 [Histórico-UI] [P] [US1] Crear test unitario frontend (marker: unit, AAA) para `SessionGuard` en `frontend/src/app/modules/cuentas-clientes/auth/guards/session.guard.spec.ts`
...
- [X] T031 [Histórico-UI] [US1] Implementar `AuthApiService` en `frontend/src/app/modules/cuentas-clientes/auth/services/auth-api.service.ts`
- [X] T032 [Histórico-UI] [US1] Implementar `SessionGuard` en `frontend/src/app/modules/cuentas-clientes/auth/guards/session.guard.ts`
- [X] T033 [Histórico-UI] [US1] Implementar pantalla de login con flujo de logout en `frontend/src/app/modules/cuentas-clientes/auth/pages/login.page.ts`

**Checkpoint**: US1 operativa y demostrable como MVP.

**US1 Gate (must pass before next story)**:
- [X] T069 [US1] Validar cumplimiento de `CA-AUT-001`, `CA-AUT-008` y `CA-AUT-009` en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
...
- [X] T040 [Histórico-UI] [P] [US2] Crear test unitario frontend (marker: unit, AAA) para `RoleGuard` en `frontend/src/app/modules/cuentas-clientes/auth/guards/role.guard.spec.ts`
...
- [X] T046 [Histórico-UI] [US2] Implementar `RoleGuard` en `frontend/src/app/modules/cuentas-clientes/auth/guards/role.guard.ts`
- [X] T047 [Histórico-UI] [US2] Implementar servicio frontend de admin usuarios/roles en `frontend/src/app/modules/cuentas-clientes/auth/services/user-role-admin.service.ts`

**Checkpoint**: US2 funcional e independiente.

**US2 Gate (must pass before next story)**:
- [X] T070 [US2] Validar cumplimiento de `CA-AUT-004` y `CA-AUT-005` en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
...
- [X] T071 [US3] Validar cumplimiento de `CA-AUT-006` en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
...
- [X] T059 [Histórico-UI] [P] [US4] Crear test unitario frontend (marker: unit, AAA) para flujo reset en `frontend/src/app/modules/cuentas-clientes/auth/services/password-reset.service.spec.ts`
...
- [X] T063 [Histórico-UI] [US4] Implementar servicio frontend de reset en `frontend/src/app/modules/cuentas-clientes/auth/services/password-reset.service.ts`
- [X] T064 [Histórico-UI] [US4] Implementar pantalla de recuperacion/cambio obligado en `frontend/src/app/modules/cuentas-clientes/auth/pages/password-reset.page.ts`

**Checkpoint**: US4 completa y verificable independientemente.

**US4 Gate (must pass before polish)**:
- [X] T072 [US4] Validar cumplimiento de `CA-AUT-003` y `CA-AUT-007` en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
...
- [X] T066 [P] Documentar resultado de prueba de performance y disponibilidad (evidencia de `CA-AUT-010`) en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/quickstart.md`
- [X] T067 [P] Actualizar mapeo RF/RNF/CA->Task IDs en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
- [X] T068 Ejecutar validacion final end-to-end de quickstart en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/quickstart.md`
- [X] T073 Validar cumplimiento final de `CA-AUT-010` (RNF-AUT-004) en `specs/003-operational/Cuentas-Clientes/autenticacion-y-rbac/backend/traceability.md`
