...
**Input**: Design documents from `specs/003-operational/Emergencias/registro-accidente/backend/`
...
- [X] T001 [Histórico-UI] Crear estructura de carpetas en `backend/apps/accidentes/{views,services,tests/{api,services,repositories}}`, `backend/core/repositories/accidentes/` y `frontend/src/app/modules/accidentes/{pages,services,guards}`
...
- [X] T005 [Bridge-FE] [P] Generar tipos TypeScript desde contrato en `frontend/src/app/modules/accidentes/services/models/accidente.types.ts` basado en `contracts/registro-accidente.openapi.yaml`
- [X] T006 [P] Crear matriz de trazabilidad CU/RF/CA→tasks en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
...
- [X] T007 Validar contrato OpenAPI como gate en `specs/003-operational/Emergencias/registro-accidente/backend/contracts/registro-accidente.openapi.yaml`
...
- [X] T032 [Histórico-UI] [P] [US1] Crear test unitario frontend (marker: unit, AAA) para `OperadorEmergenciasGuard` en `frontend/src/app/modules/accidentes/guards/operador-emergencias.guard.spec.ts`
...
- [X] T042 [Histórico-UI] [US1] Implementar `AccidenteApiService.registrar()` y `confirmarReporte()` en `frontend/src/app/modules/accidentes/services/accidente-api.service.ts`
- [X] T043 [Histórico-UI] [US1] Implementar `GeocodificacionApiService` en `frontend/src/app/modules/accidentes/services/geocodificacion-api.service.ts`
- [X] T044 [Histórico-UI] [US1] Implementar `OperadorEmergenciasGuard` en `frontend/src/app/modules/accidentes/guards/operador-emergencias.guard.ts`
- [X] T045 [Histórico-UI] [US1] Implementar página registro con manejo advertencias/retrospectivo en `frontend/src/app/modules/accidentes/pages/registro-accidente/registro-accidente.page.ts`
- [X] T045b [Histórico-UI] [US1] Implementar persistencia local (draft en localStorage), indicador de sincronización ("En vivo"/"Reconectando…"/"Sin conexión — guardado localmente") y restauración automática en `frontend/src/app/modules/accidentes/pages/registro-accidente/registro-accidente.page.ts`, según RNF-REG-006 y `.specify/docs/design/design-system.md` §2 (resiliencia de captura en campo)
- [X] T045c [Histórico-UI] [P] [US1] Crear test unitario frontend (marker: unit, AAA) para persistencia local/restauración/`syncStatus` de `registro-accidente.page.ts` en `frontend/src/app/modules/accidentes/pages/registro-accidente/registro-accidente.page.spec.ts`

**Checkpoint**: US1 operativa — MVP de entrada al camino crítico.

**US1 Gate**:
- [X] T046 [US1] Validar CA-REG-001, CA-REG-002, CA-REG-003, CA-REG-006, CA-REG-013, CA-REG-014 en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
- [X] T046b [US1] Validar RNF-REG-006 (resiliencia de captura) en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
...
- [X] T051 [Histórico-UI] [P] [US2] Crear test unitario frontend (marker: unit, AAA) para `AccidenteApiService` list/detail/patch en `frontend/src/app/modules/accidentes/services/accidente-api.service.spec.ts`
...
- [X] T054 [Histórico-UI] [US2] Implementar página lista con filtros en `frontend/src/app/modules/accidentes/pages/lista-accidentes/lista-accidentes.page.ts`
- [X] T055 [Histórico-UI] [US2] Implementar página detalle/edición en `frontend/src/app/modules/accidentes/pages/detalle-accidente/detalle-accidente.page.ts`
- [X] T056 [Histórico-UI] [US2] Registrar rutas lazy en `frontend/src/app/modules/accidentes/accidentes.routes.ts` y `frontend/src/app/app.routes.ts`

**Checkpoint**: US2 funcional e independiente de US3–US5.

**US2 Gate**:
- [X] T057 [US2] Validar CA-REG-005 y CA-REG-007 en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
...
- [X] T062 [Histórico-UI] [US3] Añadir acción descartar en `frontend/src/app/modules/accidentes/pages/detalle-accidente/detalle-accidente.page.ts` vía `AccidenteApiService.descartar()`

**Checkpoint**: US3 completa.

**US3 Gate**:
- [X] T063 [US3] Validar CA-REG-009 en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
...
- [X] T070 [Histórico-UI] [US4] Implementar modal fusión con padre preseleccionado en `frontend/src/app/modules/accidentes/pages/registro-accidente/duplicado-fusion.dialog.ts`

**Checkpoint**: US4 completa.

**US4 Gate**:
- [X] T071 [US4] Validar CA-REG-004 y CA-REG-011 en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
...
- [X] T076 [Histórico-UI] [P] [US5] Crear test unitario frontend (marker: unit, AAA) para `UnidadEmergenciaGuard` en `frontend/src/app/modules/accidentes/guards/unidad-emergencia.guard.spec.ts`
...
- [X] T081 [Histórico-UI] [US5] Implementar `UnidadEmergenciaGuard` en `frontend/src/app/modules/accidentes/guards/unidad-emergencia.guard.ts`
- [X] T082 [Histórico-UI] [US5] Implementar `AccidenteApiService.escalarSeveridad()` y UI escalamiento en `frontend/src/app/modules/accidentes/pages/detalle-accidente/escalar-severidad.panel.ts`

**Checkpoint**: US5 completa.

**US5 Gate**:
- [X] T083 [US5] Validar CA-REG-010 y CA-REG-012 en `specs/003-operational/Emergencias/registro-accidente/backend/traceability.md`
...
- [X] T086 [P] Documentar evidencia RNF-REG-005 (<2s validaciones) en `specs/003-operational/Emergencias/registro-accidente/backend/quickstart.md`
...
- [X] T091 [Histórico-UI] [P] Adoptar Tailwind CSS como sistema de utilidades CSS del frontend (`frontend/postcss.config.js` o equivalente, `frontend/src/styles.css`), documentado en `.specify/docs/infra/infrastructure.md`
- [X] T092 [Histórico-UI] [P] Implementar `AppShellComponent` (sidebar por rol agrupado, header, drawer mobile) en `frontend/src/app/shared/layout/app-shell.component.ts` y `nav-links.ts`, conforme a `design-system.md` §5
- [X] T093 [Histórico-UI] [P] Migrar `LoginPage`, `PasswordResetPage`, `HomePage` a Tailwind, manteniendo comportamiento existente
- [X] T094 [Histórico-UI] [P] Implementar `LocationPickerMapComponent` (Leaflet + OpenStreetMap, decisión documentada en `.specify/docs/infra/infrastructure.md` §6) en `frontend/src/app/shared/ui/map/location-picker-map.component.ts`, integrado en `registro-accidente.page.ts` para reemplazar la captura manual de lat/lon
- [X] T095 [Histórico-UI] [P] Implementar cascada de selección manual de ubicación (país→estado→condado→ciudad→calle, RF-REG-006 punto 3 / Escenario 5): contrato OpenAPI (`GET /accidentes/paises|estados|condados|ciudades|calles`), backend (`ubicacion_catalogo_repository.py`, `ubicacion_catalogo_service.py`, `ubicacion_catalogo_views.py`) y frontend (`ubicacion-catalogo-api.service.ts`, cascada en `registro-accidente.page.ts`)
...
- [X] T097 [Histórico-UI] Completar campos opcionales de RF-REG-002 en el formulario de registro (vehículos, heridos, víctimas, fallecidos, tipo de reporte) en `registro-accidente.page.ts`
...
- [X] T101 [Histórico-UI] Reconstruir `ListaAccidentesPage` (tabla conforme a design-system.md: header mayúsculas, badges de severidad ícono+color, columna de acción con ojo 44×44, colapso a cards en mobile, estados carga/vacío/error) y `AccidenteApiService.listar()` con objeto de filtros
- [X] T102 [Histórico-UI] [P] Crear `lista-accidentes.page.spec.ts` (no existía)
...
- [X] T104 [Histórico-UI] [P] Implementar `NotificationService` (`frontend/src/app/shared/notifications/notification.service.ts`) con colas de `toast()`/`alert()` y auto-dismiss según tabla de `design-system.md`
...
## Phase 12 (Histórico-UI — superseded by `../frontend/tasks.md` T-FE-*): RF-REG-005 UX — Detalles vs Editar e ID sin link (Session 2026-07-30)

**Purpose**: Aplicar clarificaciones de lista/workpanel: ID texto plano; ojo → modo Detalles (readonly, sin Guardar); lápiz → modo Editar (título + Guardar arriba); Descartar/Escalar siguen en Detalles.

**Independent Test**: Desde lista, ojo muestra «Detalles» sin Guardar ni inputs editables; lápiz muestra «Editar caso» + Guardar; clic en ID no navega.

- [X] T109 [Histórico-UI] [P] Lista: ID como texto plano (sin `<a>` / sin `text-accent-primary`) en tabla y cards; Acciones ojo/lápiz intactas en `lista-accidentes.page.html`
- [X] T110 [P] Detalle: título de modo (`Detalles` / `Editar caso`), ocultar Guardar y deshabilitar campos Impacto si `focus≠edit`; Guardar solo en modo Editar en `detalle-accidente.page.{ts,html}`
- [X] T111 [Histórico-UI] [P] Actualizar Jasmine: lista (ID no es link), detalle (Guardar solo con `focus=edit`, título de modo) en `*.page.spec.ts`
- [X] T112 Rebuild Docker `django`+`frontend` y verificar contenedores Up

**Checkpoint**: CA-REG-005 / RF-REG-005 Session 2026-07-30 cumplidos en UI.

---

## Phase 13 (Histórico-UI — superseded by `../frontend/tasks.md` T-FE-*): Detalles → paneles view + descartar borrador local (Session 2026-07-30)

**Purpose**: RF-REG-005 — desde Detalles, galería y datos del siniestro en solo consulta (`?mode=view`). RNF-REG-006 — CTA Descartar borrador con confirmación en lenguaje de usuario.

**Independent Test**: Ojo → «Ver galería» / «Ver datos del siniestro» sin subir/completar; lápiz conserva CTAs de captura. Registro con borrador restaurado → Descartar borrador → confirmación → form vacío.

- [X] T113 [Histórico-UI] [P] Detalle: CTAs condicionados a modo (view vs edit) + `queryParams.mode=view` en `detalle-accidente.page.html`
- [X] T114 [P] Galería: ocultar Subir/Sincronizar cuando `mode=view` en `galeria-evidencias.page.{ts,html}`
- [X] T115 [P] Enriquecimiento: modo solo lectura cuando `mode=view` en `enriquecimiento-accidente.page.{ts,html}`
- [X] T116 [P] Registro: CTA Descartar borrador + confirmación usuario + reset form en `registro-accidente.page.{ts,html}`
- [X] T117 [Histórico-UI] [P] Tests Jasmine + rebuild Docker `django`/`frontend`

**Checkpoint**: Clarificaciones Session 2026-07-30 (paneles view + borrador) en UI.
