...
**Prerrequisito:** Kafka/Pinot levantados (Docker Desktop + stack del proyecto). El bootstrap por defecto en `.env` suele ser `kafka:29092` (red Docker).

```bash
# Preferido: dentro del contenedor Django (misma red que Kafka)
docker exec -it <contenedor-django> python /app/scripts/seed_catalogos_registro.py

# Alternativa desde host con puerto publicado
cd backend
...
Esperado: 4 mensajes en `Dim_TipoReportado_topic` + 4 en `Dim_ReferenciaEstacion_topic`. Esperar ingest Pinot y verificar:

```http
GET /api/v1/accidentes/tipos-reportado
