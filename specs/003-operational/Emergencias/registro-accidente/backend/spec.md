...
> **Índice del módulo:** [`../registro-accidente.md`](../registro-accidente.md).  
> **UI Operador (Interaction Capability):** [`../frontend/spec.md`](../frontend/spec.md) — no duplicar aquí detalles de pantallas; RF-REG-005/RNF-REG-006 definen capacidad de negocio; el frontend detalla modos/CTAs.
...
- Q: ¿Patrón de Acciones en tabla (design-system vs gestionar planes)? → A: **Dos acciones por ícono** (≥44×44 px, `aria-label` y tooltip): ojo = Detalles; lápiz = Editar. Navegación a workpanel (no master-detail en la misma pantalla).
- Q: ¿Qué se puede editar en el workpanel? → A: Solo datos complementarios ya permitidos (`numvehiculos`, `numheridos`, `numfallecidos`, `descripcion` y demás vigentes). **No** clima/elementos físicos/conductores (dueño = `evidencia-unidad`).
- Q: ¿Ojo y lápiz deben distinguirse en el workpanel (título + permisos)? → A: **Sí.** Ojo → modo **Detalles**: título intuitivo de solo lectura (p. ej. «Detalles»), campos no editables, **sin** CTA Guardar cambios. Lápiz → modo **Editar**: título intuitivo de edición (p. ej. «Editar caso»), campos editables + CTA **Guardar cambios** arriba.
...
El sistema debe permitir al Operador agregar los siguientes datos complementarios (no obligatorios en el registro inicial pero útiles a distancia):
...
- **Referencia de estación (opcional):** `idreferenciaestacion` (INT, FK a `Dim_ReferenciaEstacion`) si el canal de alerta lo aporta; no implica captura de clima en sitio.
...
2. Ver en lista cada `idaccidente` como **texto plano** (sin enlace ni color de link). Un clic en el ID **no** abre el workpanel. La apertura del caso MUST ser solo vía Acciones de fila/card.
3. Ofrecer Acciones por ícono (≥44×44 px, `aria-label` y tooltip), en tabla desktop y card móvil:
   - **Ojo** → modo **Detalles**: título intuitivo de solo lectura (p. ej. «Detalles»), identidad del caso + estado, campos no editables, **sin** CTA Guardar cambios. Las acciones operativas (Descartar, Escalar, etc.) siguen disponibles según estado y rol. Los paneles adyacentes (galería de evidencia y datos del siniestro / enriquecimiento) se exponen solo como **consulta**: CTAs «Ver galería» / «Ver datos del siniestro» (no «Completar en sitio» ni captura); al navegar, esas pantallas MUST abrirse en modo view (sin CTAs de escritura).
...
**MUST NOT** ampliar edición a clima, elementos físicos, conductores, vehículos de evidencia ni fotos de campo (dueño = `evidencia-unidad`).
...
**Casos límite (lista/workpanel):** lista vacía con mensaje + CTA registrar; skeleton/error con reintentar; URL a caso ya no activo → mensaje claro y retorno a lista; Guardar sin cambios materiales no debe parecer fallo; doble envío de Guardar → un solo efecto de negocio.
...
5. Si se restaura un borrador local al abrir el formulario, MUST mostrar aviso no técnico (p. ej. «Se restauró un borrador guardado localmente») y MUST ofrecer CTA **Descartar borrador** (o equivalente). Al activarlo, MUST pedir confirmación breve en lenguaje de usuario (p. ej. «¿Descartar el borrador y empezar de nuevo?» — **sin** mencionar cache, localStorage u otros términos técnicos). Tras confirmar: eliminar el borrador local y dejar el formulario vacío/inicial. Cancelar la confirmación no altera el borrador ni el formulario.
...
- `idreferenciaestacion` (INT, FK a `Dim_ReferenciaEstacion`, opcional).
