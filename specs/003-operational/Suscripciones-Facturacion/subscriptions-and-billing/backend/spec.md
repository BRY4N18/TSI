...
**Ruta Spec Kit (esta carpeta):** `specs/003-operational/Suscripciones-Facturacion/subscriptions-and-billing/backend/`
...
| **Director de Estrategia** | Gestiona el catálogo `Dim_Plan` (crear, editar, desactivar) — RF-SUSF-001. Rol JWT: `DirectorEstrategia`. No opera suscripciones de clientes ni aprueba downgrades en v1. |
| **Administrador** | Aprueba/rechaza downgrades (RF-SUSF-003) y consulta facturación de cualquier cliente (RF-SUSF-006). **No** gestiona el catálogo de planes (ver Director de Estrategia). **No** suspende ni reactiva por mora en v1 (solo el Sistema, RF-SUSF-007). |
...
| RNF-SUSF-002 | Proveedor: solo su `idcliente`. Director de Estrategia: mutaciones de catálogo `Dim_Plan` (RF-SUSF-001). Administrador: lectura amplia de facturación + aprobaciones de downgrade (sin CRUD de planes). | Security — Control de acceso |
