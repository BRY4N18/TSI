...
- **FR-UI-002**: Página `registro-publico` — formulario CU-O18/RF-CPP-001; estados loading/error. Tras éxito, captura `demo_grant`/`idprospecto` de la respuesta y enlaza a `/ventas-crm/demo?idprospecto=…&grant=…` para continuar hacia la demo (corrección 2026-08-07 — antes se descartaba la respuesta y el flujo quedaba roto; ver `notificacion-ventas/frontend/spec.md` FR-UI-010).
...
- **FR-UI-009**: `entrada-directa` — solo Admin; formulario RF-CPP-007 / **CU-O96** (catálogo §5.9 — fuera de secuencia, ver nota en el catálogo) con **mismo chrome** Accidente (Volver link, card secciones, focus ring formularios, submit en carga, error con ícono). Sin campos de ID técnico. `tipo` restringido al enum `{Proveedor, Aseguradora, Municipio, Smart City}` (validación backend reforzada 2026-08-07). **Corrección 2026-08-08:** el formulario incluye una segunda sección obligatoria "Administrador local" (`nombres`, `apellidos`, `gmail`) — sin ella la cuenta creada quedaba sin ningún usuario que pudiera iniciar sesión.
