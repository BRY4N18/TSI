...
| Base de datos analítica | ClickHouse                   | 24.8 (`clickhouse/clickhouse-server`) | Capa analítica batch del stack `tactico` (§2.1). **Derivada**: se alimenta de Pinot vía Airflow y nunca es fuente de verdad |
| Coordinación            | Apache Zookeeper             | 3.8.4 (`zookeeper`)             | Coordinación de Kafka y Pinot                        |
| Base de datos **operacional** | Apache Pinot            | 1.2.0 (`apachepinot/pinot`)     | Almacén del modelo dimensional de dominio (`Dim_*`/`Hecho_*`) y **fuente de verdad del negocio**, como dice §1. Solo lectura desde Django: se escribe vía Kafka. ⚠️ Hasta 2026-08-23 esta fila lo rotulaba «analítica», contradiciendo a §1 y confundiéndolo con ClickHouse — que sí lo es |
