"""Idempotency-Key for red_operativa write endpoints (api-standards)."""
...
def _cache_key(scope: str, idusuario: int, idempotency_key: str) -> str:
    return f"idempotency:red_operativa:{scope}:{idusuario}:{idempotency_key}"
...
def require_idempotency_key(request: Request) -> str | None:
    """Return the header value, or None if missing."""
