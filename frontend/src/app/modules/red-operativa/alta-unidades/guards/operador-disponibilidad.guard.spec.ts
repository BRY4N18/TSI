/** @marker unit */
import { TestBed } from '@angular/core/testing';
import { Router } from '@angular/router';

import { operadorDisponibilidadGuard } from './operador-disponibilidad.guard';
import { AuthApiService } from '../../../cuentas-clientes/auth/services/auth-api.service';

describe('operadorDisponibilidadGuard', () => {
  it('allows_operador_role', () => {
    // Arrange
    TestBed.configureTestingModule({
      providers: [
        {
          provide: AuthApiService,
          useValue: { isAuthenticated: () => true, hasRole: (r: string) => r === 'Operador' },
        },
        { provide: Router, useValue: { createUrlTree: () => false } },
      ],
    });

    // Act
    const result = TestBed.runInInjectionContext(() =>
      operadorDisponibilidadGuard({} as never, {} as never),
    );

    // Assert
    expect(result).toBe(true);
  });

  it('blocks_administrador_role', () => {
    // Arrange
    let redirected = false;
    TestBed.configureTestingModule({
      providers: [
        {
          provide: AuthApiService,
          useValue: { isAuthenticated: () => true, hasRole: (r: string) => r === 'Administrador' },
        },
        { provide: Router, useValue: { createUrlTree: () => (redirected = true) } },
      ],
    });

    // Act
    TestBed.runInInjectionContext(() => operadorDisponibilidadGuard({} as never, {} as never));

    // Assert
    expect(redirected).toBe(true);
  });
});
