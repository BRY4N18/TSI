/**
 * Una sola página para los ocho listados.
 *
 * Los ocho se diferencian **solo** en su declaración de columnas y filtros.
 * Ocho ficheros serían ocho copias del mismo `template`, y la novena copia sería
 * la que se olvidara del aviso de error.
 *
...
import { INFORMES_CUENTAS } from '../../definiciones/informes-cuentas.definiciones';
...
@Component({
  ...
  template: `
    <section [class]="shellClass">
      <a
        routerLink="/cuentas-clientes/informes"
...
export class InformeCuentasPage implements OnInit {
  ...
  readonly definicion = computed(() => {
    ...
    const definicion = INFORMES_CUENTAS[id];
    if (!definicion) {
