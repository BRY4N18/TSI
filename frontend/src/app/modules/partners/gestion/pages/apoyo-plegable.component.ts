...
@Component({
  selector: 'app-apoyo-plegable-partners',
  standalone: true,
          ...
          <article class="rounded-md border border-border-default bg-bg-page p-3">
            ...
            @switch (bloque.carga.estado) {
              ...
              @default {
                ...
                @if (bloque.informe === 'participacion-ingresos-api') {
                  <ul class="m-0 mt-2 flex list-none flex-col gap-1 p-0">
                    @for (fila of bloque.carga.data; track $index) {
                      <li class="text-sm text-text-secondary" data-testid="excedente-vs-base">
                        {{ texto(fila['partner']) }} · base
                        {{ num(fila['ingreso_base']) }} · excedente
                        {{ num(fila['excedente']) }}
                      </li>
                    }
                  </ul>
...
export class ApoyoPlegableComponent {
  ...
  resumen(bloque: BloqueApoyo): string {
    ...
    if (bloque.informe === 'metricas-consumo') {
      return `${filas.length} partners · no éxito, no «fallo de servicio»`;
    }
    if (bloque.informe === 'reporte-mensual-consumo') {
      return `${filas.length} filas de reporte mensual`;
    }
    if (bloque.informe === 'consumo-por-endpoint') {
      return `${filas.length} endpoints`;
    }
    if (bloque.informe === 'participacion-ingresos-api') {
      return 'Excedente aparte del ingreso base';
    }
    if (bloque.informe === 'tasa-rechazo-produccion') {
      return `${filas.length} motivos de rechazo`;
    }
