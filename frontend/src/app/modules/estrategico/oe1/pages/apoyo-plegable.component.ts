...
import { CargaInforme, num, texto } from '../models/informes-oe1.types';
...
@Component({
  selector: 'app-apoyo-plegable-oe1',
  standalone: true,
  imports: [DecimalPipe],
        ...
        @for (bloque of bloques(); track bloque.informe) {
          <article class="rounded-md border border-border-default bg-bg-page p-3">
            ...
            @switch (bloque.carga.estado) {
              ...
              @default {
                @if (bloque.informe === 'tasa-renovacion') {
                  <ul class="m-0 mt-2 flex list-none flex-col gap-1 p-0">
                    @for (fila of bloque.carga.data; track $index) {
                      <li class="text-sm text-text-secondary">
                        {{ num(fila['renovadas']) | number }} renovadas de
                        {{ num(fila['vencidas']) | number }} vencidas · tasa
                        {{ num(fila['tasa_renovacion']) }}
                      </li>
                    }
                  </ul>
                } @else if (bloque.informe === 'velocidad-ciclo-venta') {
                  <ul class="m-0 mt-2 flex list-none flex-col gap-1 p-0">
                    @for (fila of bloque.carga.data; track $index) {
                      <li class="text-sm text-text-secondary">
                        {{ texto(fila['etapa']) }} · ejecutivo
                        {{ texto(fila['idejecutivo']) }} ·
                        {{ num(fila['segundos_promedio']) | number: '1.0-0' }} s
                      </li>
                    }
                  </ul>
                } @else {
