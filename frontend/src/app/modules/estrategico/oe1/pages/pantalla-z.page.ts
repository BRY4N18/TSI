...
import {
  ...
  num,
  texto,
} from '../models/informes-oe1.types';
import { InformesOe1ApiService } from '../services/informes-oe1-api.service';
...
export class PantallaZPage {
  private readonly api = inject(InformesOe1ApiService);
...
function metricaNula(env: EnvelopeInforme, informe: string): boolean {
  ...
  if (informe === 'churn-por-cohorte') {
    return data.every((f) => num(f['pct_churn']) === null);
  }
  return false;
}
...
function etiquetaApoyo(informe: string): string {
  switch (informe) {
    case 'tasa-renovacion':
      return 'Renovación: denominador = vencidas';
    case 'velocidad-ciclo-venta':
      return 'Velocidad por etapa y ejecutivo';
    default:
      return informe;
  }
}
