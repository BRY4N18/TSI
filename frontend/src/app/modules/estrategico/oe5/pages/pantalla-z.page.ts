...
import { definicionDe, informesDe } from '../definiciones/pantallas-oe5.definiciones';
...
import {
  ...
  num,
  texto,
} from '../models/informes-oe5.types';
import { InformesOe5ApiService } from '../services/informes-oe5-api.service';
...
export class PantallaZPage {
  private readonly api = inject(InformesOe5ApiService);
...
function metricaNula(env: EnvelopeInforme, informe: string): boolean {
  ...
  if (informe === 'cumplimiento-sla') {
    return data.every((f) => (num(f['con_compromiso']) ?? 0) === 0);
  }
  return false;
}
...
function etiquetaApoyo(informe: string): string {
  switch (informe) {
    case 'rendimiento-por-agente':
      return 'Carga por agente (cola, no desempeño)';
    case 'reincidencia-soporte':
      return 'Reincidencia cliente × servicio';
    default:
      return informe;
  }
}
