...
import { CargaInforme, num, texto } from '../models/informes-oe6.types';
...
@Component({
  selector: 'app-apoyo-plegable-oe6',
  standalone: true,
  imports: [DecimalPipe],
        ...
        @for (bloque of bloques(); track bloque.informe) {
          <article class="rounded-md border border-border-default bg-bg-page p-3">
            ...
            @switch (bloque.carga.estado) {
              ...
              @default {
                @if (bloque.informe === 'rechazo-y-timeout-por-unidad') {
                  <ul class="m-0 mt-2 flex list-none flex-col gap-1 p-0">
                    @for (fila of bloque.carga.data; track $index) {
                      <li class="text-sm text-text-secondary">
                        {{ texto(fila['unidad']) }} ·
                        {{ num(fila['rechazados']) | number }} /
                        {{ num(fila['ofrecidos']) | number }} ofrecidos
                      </li>
                    }
                  </ul>
                } @else {
