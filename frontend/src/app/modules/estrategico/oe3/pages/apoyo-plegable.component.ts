...
import { CargaInforme, num, texto } from '../models/informes-oe3.types';
...
@Component({
  selector: 'app-apoyo-plegable-oe3',
  standalone: true,
  imports: [DecimalPipe],
        ...
        @for (bloque of bloques(); track bloque.informe) {
          <article class="rounded-md border border-border-default bg-bg-page p-3">
            ...
            @switch (bloque.carga.estado) {
              ...
              @default {
                @if (bloque.informe === 'perdida-de-senal') {
                  <ul class="m-0 mt-2 flex list-none flex-col gap-1 p-0">
                    @for (fila of bloque.carga.data; track $index) {
                      <li class="text-sm text-text-secondary">
                        {{ texto(fila['unidad']) }} ·
                        {{ num(fila['huecos']) | number }} /
                        {{ num(fila['intervalos_medidos']) | number }} intervalos
                      </li>
                    }
                  </ul>
                } @else {
