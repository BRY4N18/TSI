...
import { definicionDe, informesDe } from '../definiciones/pantallas-oe3.definiciones';
...
import {
  ...
  num,
  texto,
} from '../models/informes-oe3.types';
import { InformesOe3ApiService } from '../services/informes-oe3-api.service';
...
export class PantallaZPage {
  private readonly api = inject(InformesOe3ApiService);
  ...
  esSinCapacidad(fila: Record<string, unknown>): boolean {
    return num(fila['sin_capacidad']) === 1;
  }
...
function metricaNula(env: EnvelopeInforme, informe: string): boolean {
  ...
  if (informe === 'latencia-asignacion') {
    return data.every((f) => num(f['p95_min']) === null);
  }
  return false;
}
...
function etiquetaApoyo(informe: string): string {
  switch (informe) {
    case 'perdida-de-senal':
      return 'Pérdida de señal (con recuento)';
    default:
      return informe;
  }
}
