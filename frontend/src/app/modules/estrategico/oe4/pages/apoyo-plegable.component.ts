...
import { CargaInforme, num, texto } from '../models/informes-oe4.types';
...
@Component({
  selector: 'app-apoyo-plegable-oe4',
  standalone: true,
  imports: [DecimalPipe],
        ...
        @for (bloque of bloques(); track bloque.informe) {
          <article class="rounded-md border border-border-default bg-bg-page p-3">
            ...
            @switch (bloque.carga.estado) {
              ...
              @default {
                @if (bloque.informe === 'calidad-por-origen') {
                  <ul class="m-0 mt-2 flex list-none flex-col gap-1 p-0">
                    @for (fila of bloque.carga.data; track $index) {
                      <li class="text-sm text-text-secondary">
                        {{ texto(fila['origen']) }} ·
                        {{ num(fila['pct_completitud']) | number: '1.0-2' }} ·
                        {{ num(fila['casos']) | number }} casos
                      </li>
                    }
                  </ul>
                } @else {
