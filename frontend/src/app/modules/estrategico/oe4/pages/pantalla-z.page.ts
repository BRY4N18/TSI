...
import { definicionDe, informesDe } from '../definiciones/pantallas-oe4.definiciones';
...
import {
  ...
  num,
  texto,
} from '../models/informes-oe4.types';
import { InformesOe4ApiService } from '../services/informes-oe4-api.service';
...
export class PantallaZPage {
  private readonly api = inject(InformesOe4ApiService);
  ...
  cumple(carga: CargaInforme): boolean | null {
    return cumpleDe(carga.meta);
  }

  esSinMasaCritica(fila: Record<string, unknown>): boolean {
    return num(fila['sin_masa_critica']) === 1;
  }
...
function metricaNula(env: EnvelopeInforme, informe: string): boolean {
  ...
  if (informe === 'indice-calidad-historico') {
    return data.every((f) => num(f['indice_consolidado']) === null);
  }
  return false;
}
...
function etiquetaApoyo(informe: string): string {
  switch (informe) {
    case 'calidad-por-origen':
      return 'Calidad por origen (central vs campo)';
    default:
      return informe;
  }
}
