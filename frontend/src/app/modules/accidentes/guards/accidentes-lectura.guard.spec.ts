...
import { accidentesLecturaGuard } from './accidentes-lectura.guard';

describe('accidentesLecturaGuard', () => {
  ...
  it('redirects unauthenticated users to login', () => {
    ...
    const result = TestBed.runInInjectionContext(() =>
      accidentesLecturaGuard({} as never, {} as never),
    );
  ...
  it('redirects users without a reading role to access denied', () => {
    // Arrange
    ...
    expect(authApi.hasAnyRole).toHaveBeenCalledWith(['Operador', 'Tecnico', 'Administrador']);
  });

  it('allows access when user has Tecnico role', () => {
    // Arrange
    authApi.isAuthenticated.and.returnValue(true);
