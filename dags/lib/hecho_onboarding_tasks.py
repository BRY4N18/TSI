"""Tareas del flujo de `hecho_onboarding`."""
...
from lib.hechos import hecho_onboarding
from lib.tipos_almacen import ajustar_tipos

FLUJO = "hecho_onboarding"
FUENTES = ("onboarding", "clientes")
...
def extract(ts: str, **_) -> None:
    ...
    datos = hecho_onboarding.extraer()
...
def transform(ts: str, **_) -> None:
    ...
    guardar(hecho_onboarding.construir(datos, _ahora()), ruta(ts, "transform", _prefijo("")))


def load(ts: str, **_) -> None:
    filas = ajustar_tipos("hecho_onboarding", cargar(ruta(ts, "transform", _prefijo(""))))
    cargar_particiones("hecho_onboarding", filas)
