"""Tareas del flujo de `hecho_llamada_api`."""
...
from lib.hechos import hecho_llamada_api
from lib.tipos_almacen import ajustar_tipos

FLUJO = "hecho_llamada_api"
FUENTES = ("llamadas", "partners", "credenciales")
...
def extract(ts: str, **_) -> None:
    ...
    datos = hecho_llamada_api.extraer()
...
def transform(ts: str, **_) -> None:
    ...
    guardar(hecho_llamada_api.construir(datos, _ahora()), ruta(ts, "transform", _prefijo("")))


def load(ts: str, **_) -> None:
    filas = ajustar_tipos("hecho_llamada_api", cargar(ruta(ts, "transform", _prefijo(""))))
    cargar_particiones("hecho_llamada_api", filas)
